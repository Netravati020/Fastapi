...
from fastapi import FastAPI, Depends, status, HTTPException, Response, Request, Form, File, UploadFile, Cookie, Header
...
def read_emps(db: Session = Depends(get_db), current_user: schema.User=Depends(get_current_user)):
    ...
    # emp=db.execute("SELECT * FROM employees LIMIT 100, OFFSET 15")
    return (emp[15:100], current_user)



# delete perticular employee detail
@app.delete('/{employee_id}', status_code=status.HTTP_204_NO_CONTENT, tags=['Employees'])
def update(employee_id,  db:Session=Depends(get_db)):
